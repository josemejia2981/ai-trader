...
# Selector de contratos v2 institucional
# Prioridad: Schwab (greeks reales) → yfinance (fallback)
...
# Schwab provider — greeks reales cuando hay credenciales
try:
    from agents.schwab_provider import get_options_chain as schwab_options, get_realtime_quote, is_available as schwab_available
    _SCHWAB_OK = schwab_available()
except Exception:
    _SCHWAB_OK = False
    schwab_options = lambda *a, **kw: []
    get_realtime_quote = lambda s: None

if _SCHWAB_OK:
    print("[SCHWAB] Datos en tiempo real activados")
else:
    print("[SCHWAB] Sin credenciales — usando yfinance (datos retrasados 15-20 min)")
...
try:
    ...
    MAX_ENTRY_PRICE = 30.0
    MAX_RISK_PER_TRADE = 800
    MAX_OTM_PCT = 0.10
    MAX_STRIKE_DISTANCE_PCT = 0.15
    STOP_LOSS_PCT = 0.40
...
def build_levels(opt_type, up, strike, ep):
    ...
    if opt_type == "CALL":
        se = round(up, 2)
        ss = round(up * 0.97, 2)
        st1 = round(max(sp, up * 1.04), 2)
        st2 = round(up * 1.08, 2)
    else:
        se = round(up, 2)
        ss = round(up * 1.03, 2)
        st1 = round(min(sp, up * 0.96), 2)
        st2 = round(up * 0.92, 2)
    ...
    return {
        ...
        "option_trailing_stop": opt_trail,
        "max_option_entry": max_entry,
        "stock_entry_price": se,
        "stock_stop_loss": ss,
        "stock_take_profit_1": st1,
        "stock_take_profit_2": st2,
    }
...
def score_contract(row, price, opt_type):
    ...
    dte = si(row.get("dte"))
    iv = sf(row.get("impliedVolatility"), 0.30)
    ...
    return {
        ...
        "stop_loss": opt_stop,
        "option_stop_loss": opt_stop,
        "take_profit": lv.get("option_take_profit_2"),
        "option_take_profit": lv.get("option_take_profit_2"),
        "take_profit_1": lv.get("option_take_profit_1"),
        "take_profit_2": lv.get("option_take_profit_2"),
        "trailing_stop": lv.get("option_trailing_stop"),
        "max_option_entry": lv.get("max_option_entry"),
        "stock_entry_price": lv.get("stock_entry_price"),
        "stock_stop_loss": lv.get("stock_stop_loss"),
        "stock_take_profit_1": lv.get("stock_take_profit_1"),
        "stock_take_profit_2": lv.get("stock_take_profit_2"),
        "risk_amount": risk_amt,
        "potential_profit": pot_prof,
        "risk_reward": rr,
        "contract_quality_score": sc,
        "score": sc,
        "recommendation": rec,
    }
...
def _get_price(symbol):
    """Precio actual: Schwab (tiempo real) → yfinance (retrasado)."""
    if _SCHWAB_OK:
    ...
    return sf(hist["Close"].iloc[-1]) if not hist.empty else 0.0


def _schwab_candidates(symbol, opt_type, price):
    """Contratos con greeks REALES de Schwab."""
    raw = schwab_options(symbol, opt_type, MIN_DTE, MAX_DTE)
    if not raw:
        return pd.DataFrame()

    enriched = []
    for c in raw:
        strike = sf(c.get("strike"))
        dist   = abs(strike - price) / price if price > 0 else 1.0

        if dist > MAX_STRIKE_DISTANCE_PCT:
            continue
        if opt_type == "CALL" and strike > price * (1 + MAX_OTM_PCT):
            continue
        if opt_type == "PUT" and strike < price * (1 - MAX_OTM_PCT):
            continue

        # Greeks REALES — no necesitamos estimarlos
        real_delta = abs(sf(c.get("delta", 0)))
        if real_delta < MIN_DELTA:
            continue
        if sf(c.get("spread_pct", 100)) > MAX_SPREAD_PCT:
            continue
        if si(c.get("openInterest", 0)) < MIN_OPEN_INTEREST:
            continue
        if si(c.get("volume", 0)) < MIN_VOLUME:
            continue

        ep = sf(c.get("entry_price", 0) or c.get("mid_price", 0))
        if ep <= 0 or ep > MAX_ENTRY_PRICE:
            continue
        ...
        if risk_amt > MAX_RISK_PER_TRADE:
            continue

        # Score de calidad del contrato
        oi  = si(c.get("openInterest", 0))
        vol = si(c.get("volume", 0))
        spct = sf(c.get("spread_pct", 100))
        dte  = si(c.get("dte", 0))
        sc = 0
        if real_delta >= 0.65:  sc += 25
        elif real_delta >= 0.55: sc += 20
        elif real_delta >= 0.45: sc += 14
        if oi >= 5000:  sc += 15
        elif oi >= 2000: sc += 12
        elif oi >= 1000: sc += 9
        ...
        enriched.append({
            **c,
            ...
            "delta": c.get("delta", 0),
            "delta_estimate": real_delta,
            "entry_price": ep,
            "stop_loss": opt_stop,
            "take_profit": opt_tp2,
            "take_profit_2": opt_tp2,
            "option_stop_loss": opt_stop,
            "option_take_profit": opt_tp2,
            "risk_amount": risk_amt,
            "potential_profit": pot_prof,
            "risk_reward": rr,
            "contract_quality_score": sc,
    ...
    return df.sort_values(
        by=["contract_quality_score", "delta", "risk_reward", "openInterest", "volume"],
        ascending=[False, False, False, False, False],
    )


def _yfinance_candidates(symbol, opt_type, price):
    """Fallback: contratos de yfinance con delta estimado."""
    ticker = yf.Ticker(symbol)
    exps = ticker.options
    if not exps:
        return pd.DataFrame()
    ...
    for _, row in contracts.iterrows():
        ...
        if scored is None:
            continue
        if abs(scored.get("delta", 0)) < MIN_DELTA:
            continue
        if scored.get("spread_pct", 100) > MAX_SPREAD_PCT:
            continue
        if si(row.get("openInterest")) < MIN_OPEN_INTEREST:
            continue
        if si(row.get("volume")) < MIN_VOLUME:
            continue
        if scored.get("entry_price", 0) > MAX_ENTRY_PRICE:
            continue
        if scored.get("risk_amount", 999999) > MAX_RISK_PER_TRADE:
            continue
...
def get_candidates(symbol, opt_type="CALL"):
    ...
    if _SCHWAB_OK:
        df = _schwab_candidates(symbol, opt_type, price)
        if not df.empty:
            return df
        print("[SCHWAB] Sin contratos válidos para " + symbol + " — intentando yfinance...")

    return _yfinance_candidates(symbol, opt_type, price)
...
def clear_contract_state(state, reason):
    state.update({
        "best_contract": None,
...
def option_contract_agent(state):
    ...
    option_strategy = state.get("option_strategy", "NO TRADE")
    institutional_signal = state.get("institutional_signal", False)
    ...
    # ==================================================
    if (
        not institutional_signal
        or not trade_allowed
        or option_strategy == "NO TRADE"
        or signal == "HOLD"
        or direction == "NONE"
    ):
        print("  Contrato: OMITIDO (NO TRADE)")
        return clear_contract_state(
            state,
            "Búsqueda de contrato omitida: no hay señal institucional válida."
        )

    if not symbol:
        print("  Contrato: OMITIDO (sin símbolo)")
        return clear_contract_state(state, "No symbol.")

    text = (direction + " " + signal + " " + str(state.get("trend", "")) + " " + str(state.get("entry_type", ""))).upper()
    ...
    try:
        ...
        if best is None:
            print("  Sin contrato válido.")
            return clear_contract_state(
                state,
                "Sin contrato valido. Criterios: delta>="
                + str(MIN_DELTA)
                + ", spread<="
                + str(MAX_SPREAD_PCT)
                + "%, OI>="
                + str(MIN_OPEN_INTEREST)
                + ", vol>="
                + str(MIN_VOLUME)
                + ", prima<=$"
                + str(MAX_ENTRY_PRICE)
        ...
        fields = [
            ...
            "impliedVolatility", "delta_estimate", "delta", "entry_price",
            "max_option_entry", "stop_loss", "take_profit", "take_profit_1",
            "take_profit_2", "trailing_stop", "option_stop_loss",
            "option_take_profit", "stock_entry_price", "stock_stop_loss",
            "stock_take_profit_1", "stock_take_profit_2", "risk_amount",
            "potential_profit", "risk_reward", "distance_pct",
        ...
        print("  Error contrato: " + str(e))
        return clear_contract_state(state, "Error: " + str(e))